...
import { QUEUE_NAMES, getMetricsQueue, getHealthChecksQueue, getCleanupQueue } from '@/lib/queues'
...
let workers: Array<{ name: string; worker: unknown; scheduler?: QueueScheduler }> = []
...
export async function shutdownWorkers(): Promise<void> {
  ...
  const shutdownPromises = workers.map(async ({ name, worker, scheduler }) => {
    try {
      if (scheduler) {
